...
export async function listAllBooks(params: ParamsListBooks) {
  ...
  const where: Prisma.BookWhereInput = {
    ...
    ...(authorName
      ...
      : {}),
    ...(tag && tag.length > 0
      ? { tags: { some: { name: { in: tag, mode: "insensitive" } } } }
      : {}),
  };
...
export async function updateBook(inputData: UpdateBookSchema) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!bookFound)
      throw new CustomError("Book not found", StatusCode.NOT_FOUND);
    const finalStatus = status ?? bookFound.status;
    if (rating !== undefined && finalStatus !== Book_Status.read)
      throw new CustomError(
        "cannot set a rating unless the book has been read",
        StatusCode.CONFLICT,
      );
    ...
    if (status !== undefined && status !== bookFound.status) {
      logger.info(
        `book ${bookId} status changed: ${bookFound.status} -> ${status}`,
      );
    }
    return updatedBook;
  });
...
export async function getStats() {
  ...
  const byStatus: Record<string, number> = Object.fromEntries(
    Object.values(Book_Status).map((s) => [s, 0]),
  );
  byStatusRaw.forEach((r) => {
    byStatus[r.status] = r.count;
  });

  const byGenre: Record<string, number> = Object.fromEntries(
    Object.values(Book_Genre).map((g) => [g, 0]),
  );
  byGenreRaw.forEach((r) => {
    byGenre[r.genre] = r._count;
  });
